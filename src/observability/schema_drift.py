...
- Data type changes
- Constraint changes (nullable → not null)
...
class DriftReport(BaseModel):
    ...
    severity: DriftSeverity = Field(
        default=DriftSeverity.WARNING,
        description="Overall severity of the drift"
    )
    
    breaking_change: bool = Field(
        default=False,
        description="Whether any change is breaking"
    )
    
    affected_downstream: List[str] = Field(
        default_factory=list,
        description="Downstream models/tables affected"
    )
    
    business_impact: str = Field(
        default="",
        description="LLM-generated business impact assessment"
    )
    ...
    confidence_score: float = Field(
        default=0.0,
        ge=0.0, le=1.0,
        description="LLM confidence in assessment"
    )
    ...
    def _is_breaking(self, changes: List[SchemaChange]) -> bool:
        ...
        for change in changes:
            if change.change_type in breaking_types:
                return True
            # Nullable → Not Null is breaking
            if (change.change_type == ChangeType.NULLABLE_CHANGED 
...
"""
    ...
    def format_report(self, report: DriftReport) -> str:
        ...
        changes_text = "\n".join([
            f"  - {c['change_type']}: {c['column_name']} ({c.get('old_value', 'N/A')} → {c.get('new_value', 'N/A')})"
        ...
        return f"""
{severity_emoji.get(report.severity, "❓")} **Schema Drift Report: {report.table_name}**
Source: {report.source_system} | Detected: {report.detected_at.strftime('%Y-%m-%d %H:%M UTC')}

**Changes:**
{changes_text}

**Breaking Change:** {'🔴 YES' if report.breaking_change else '🟢 NO'}
