...
class SQLServerIngestion:
    ...
    def _validate_referential_integrity(
        ...
        valid_items = order_items_df.join(order_ids, on="order_id", how="inner")

        invalid_items = order_items_df.join(order_ids, on="order_id", how="left_anti")
    ...
    def ingest_order_items(
        ...
        with PipelineContext("sqlserver_order_items_ingestion", source_table="dbo.OrderItems"):
            ...
            if validate_orders:
                ...
                orders_df = (
                    self._read_mock_data("orders")
                    if not self.settings.enable_real_database_connections
                    else None
                )
...
def main():
    ...
    parser.add_argument("--table", choices=["orders", "order_items", "all"], default="all")
    ...
    if args.table == "all":
        ...
        for table, result in results.items():
            print(
                f"{table}: {result['rows_affected']} rows, {result.get('quarantined_count', 0)} quarantined"
            )
        ...
        print(
            f"order_items: {result['rows_affected']} rows, {result.get('quarantined_count', 0)} quarantined"
        )
