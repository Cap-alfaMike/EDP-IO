...
class BronzeWriter:
    ...
    def _validate_schema(
        ...
        extra = actual_columns - expected_columns - {"_ingestion_timestamp", "_source_system", "_batch_id", "_file_path"}
        if extra:
            logger.warning(
    ...
    def write(
        ...
        with PipelineContext(
            ...
            df_with_metadata = self._add_metadata_columns(
                df, source_system, batch_id, file_path
            )
            ...
            if partition_columns is None:
                ...
                df_with_metadata = df_with_metadata.withColumn(
                    "_ingestion_date",
                    F.to_date(F.col("_ingestion_timestamp"))
                )
            ...
            if mode == WriteMode.OVERWRITE:
                result = self._write_overwrite(
                    df_with_metadata, table_path, partition_columns
                )
